...
Ревью схемы почти везде продаётся. Бот, который разбирает каждый пулл-реквест, анализ, который прослеживает значение между файлами, реестр, ловящий расхождение схемы с миграциями, советы по индексам на основе реальной статистики таблиц — обычно всё это лежит за подпиской: за место в команде или за базу.
...
|---|---|
| Бот ревью PR — пишет один раз, дальше обновляет тот же комментарий | [`blast-radius`](../rules/blast-radius.md) + Action |
| Анализ, который ведёт queryset через границы функций | [`nplusone`](../rules/nplusone.md) |
| Реестр схемы и детект дрейфа | [`drift`](../rules/drift.md) |
| Предложения индексов по реальной статистике таблиц | `suggest-indexes` + `--stats` |
